...
//      indextype->getsize()  /* body of from_state */
	...
	"    stringstream cout (stringstream::out);\n"	       		  
          "    for (int i = 0; i < %d; i++) {\n"
          // JESSE: this appears to be a bug; this opertor string()
          //        should not be printing anything, just constructing
          //        a string object.
          // "      array[i].print(); } return cout.str();};\n\n",
          // JESSE: Here's my fix...
           "      cout << (string)(array[i]) ; } return cout.str();};\n\n",
		  indextype->getsize(),
          indextype->getsize());
  ...
  if (!declared)
    ...
    fprintf(codefile,
     ...
     fprintf(codefile,"    %s.print();\n", f->getvalue()->generate_code() );
       ...
       fprintf(codefile, "  operator string() {\n");
       fprintf(codefile,"  string s;\n");
       for( f = fields; f != NULL; f = f->getnext() )
     fprintf(codefile,"    s = s + (string)%s;\n", f->getvalue()->generate_code() );
...
void make_op_str_aux (vardecl *var)
{
  fprintf(codefile,"s = s + (string)%s;\n",
      var->generate_code() );
}
...
void make_op_str(ste *globals)
     /* done recursively. */
{
  fprintf(codefile,
      "world_class::operator string()\n"
      "{\n"
      "  static int num_calls = 0; /* to ward off recursive calls. */\n"
      "  if ( num_calls == 0 ) {\n"
      "    num_calls++;\n"
      "  string s;\n");
  map_vars(globals, &make_op_str_aux );
  fprintf(codefile,
      "    num_calls--;\n"
      "  return s;}\n}\n");
}
