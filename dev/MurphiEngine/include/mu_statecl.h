...
private:
  union {
    ...
    unsigned int lv;   // state number used in trace info file
  };
...
public:
  ...
  {
#ifdef POINTER_TRICK
	bits = (BIT_BLOCK*)malloc((size_t) BLOCKS_IN_WORLD);
#endif
...
#ifdef HASHC
friend class hash_function;     // "class" added by SPARK
// friend unsigned int* hash_function::hash(state*, bool); // tried by SPARK
// friend bool StateEquivalent(state * l, state * r);   // Uli: not necessary
  ...
  {
    unsigned int sum = 0;
    unsigned int *pt = (unsigned int *)bits;
...
#ifdef POINTER_TRICK_BLAH
	  state & operator = (const state & other)
    {
		if (this != &other) {
	bits = (BIT_BLOCK*)malloc((size_t) BLOCKS_IN_WORLD);
    memcpy(bits, other.bits, BLOCKS_IN_WORLD);
  ...
  //{ return ( memcmp(bits, other.bits, BLOCKS_IN_WORLD) == 0); };
  { printf("unimplemented function in Murphi Engine\n");exit(1);return (false);}
  inline bool operator != (state& other) const
  //{ return ( memcmp(&bits, &other.bits, BLOCKS_IN_WORLD) != 0); };
  //{ return ( memcmp(bits, other.bits, BLOCKS_IN_WORLD) != 0); };
  { printf("unimplemented function in Murphi Engine\n");exit(1);return (false);}
  ...
  void Normalize();
#ifdef ERLANG
  void Exhaustive_Fast_Canonicalize();
  void Heuristic_Fast_Canonicalize();
#endif
