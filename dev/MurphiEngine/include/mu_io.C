...
#define PRINT_1000000_FLAG "-p6" /* Guess, just guess. */
#define PRINT_10000000_FLAG "-p7" /* Guess, just guess. */
...
void argclass::ProcessOptions(string_iterator *options)
{
  ...
  for ( options->start();
	...
	options->next()  )
    {
      ...
      if ( strcmp( option, PRINT_1000000_FLAG ) == 0 )
        {
          progress_count.set(1000000);
          print_progress.set(TRUE);
          continue;
        }
      if ( strcmp( option, PRINT_10000000_FLAG ) == 0 )
        {
          progress_count.set(10000000);
          print_progress.set(TRUE);
          continue;
        }
...
{
  static unsigned int statenum = 1;
  cout << "State " << statenum++ << ":\n";
  theworld.print();
  cout << '\n';
}
...
unsigned int TraceFileManager::numLast()
{
  return last;
}
...
unsigned int TraceFileManager::readLong(int bytes)
{
  unsigned int ret=0;
  int g;
...
const TraceFileManager::Buffer* TraceFileManager::read(unsigned int number)
{
  if (number!=inBuf) 
  {
