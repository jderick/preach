...
public:
  StateManager(bool createqueue, unsigned int NumStates);
  ~StateManager();
  ...
  unsigned int NumEltsReduced();   // Uli
  unsigned int QueueNumElts();
...
public:
  ...
  void AllStartStates();
#ifdef ERLANG
  int AllStartStatesList(char*); // for preach
  int StartStateName( char* l,unsigned ss_num);
#endif
...
class RuleManager
{
  ...
  unsigned int rules_fired;
  unsigned int * NumTimesFired; /* array for storing the number
...
#ifdef ERLANG
  int has_cangetto(char*, char*);
  int is_p_state(char*, char*);
  int is_q_state(char*, char*);
  int CheckInvariantsErl(char*, char*);
  int StateToString(char*, char*);
  int PrintDiff(char*, char*);
  int model_name(char*, char*);
#endif
...
#endif
unsigned int NumCurState;     // Uli: number of the current state for trace 
                               //      info file
