...
/****************************************
  ...
  ****************************************/
dynBitVec::dynBitVec( unsigned int nBits )
: numBits( nBits )
{
...
StatePtr::StatePtr(unsigned int l) { lCheck(); lv=l; }

void StatePtr::set(state* s) { sCheck(); sp=s; }
void StatePtr::set(unsigned int l) { lCheck(); lv=l; }
void StatePtr::clear() {
#ifdef HASHC
  if (args->trace_file.value) 
    lv=0;
...
unsigned int StatePtr::lVal() { lCheck(); return lv; }
...
#ifdef HASHC
  if (args->trace_file.value) {
    unsigned int *key = h3->hash(s, FALSE);
    unsigned int c1 = key[1] &
      ((~0UL) << (args->num_bits.value>32 ? 0 : 32-args->num_bits.value));
    unsigned int c2 = key[2] &
...
#else
  BIT_BLOCK *tmp;
  memcpy(l->bits, r->bits, BLOCKS_IN_WORLD);
  tmp = l->bits;
  *l = *r;
  l->bits = tmp;
#endif
...
/****************************************
  ...
  ****************************************/
state_queue::state_queue( unsigned int mas )
...
#elsifndef HASHC
  return sizeof(state*);      //***spark   /* Full ver: only a ptr to state. */
#else
  return sizeof(state);       //***spark   // Hash compaction
#endif                        //***spark   // Hack to minimize the change, isolated in only one file
}
...
{
  unsigned int i;
  unsigned int ind = front;
  for( i = 1; i < num_elts; i++ )
    {
      // convert to print in unsigned int format?
      cout << "State " << i << " [" << ind << "]:\n";
...
#else 
  //  printf ("size of Unsigned32: %d\n", sizeof(Unsigned32));  //spark
  assert (sizeof(Unsigned32)==4);   // the implementation is pretty depen-
                                    // dent on the 32 bits
  unsigned int size =
    (unsigned int)(((double)table_size)*((double)args->num_bits.value)/32) + 3;
    // higher precision necessary to avoid overflow
    // two extra elements needed in table
  printf("table = new Unsigned32[%u], num_bits.value = %u, table_size=%u\n",size,args->num_bits.value,table_size);
...
// JESSE: just copied this from StateManager::harmonic()
{
  return (n<1) ? 0 :
                 log(n) + 0.577215665 + 1/(2*n) - 1/(12*n*n);
}
double state_set::ProbAtLeastOneOmittedState()
{
  // JESSE: created this function based on StateManager::PrintProb()
...
{
#ifndef HASHC
  unsigned int key = in->hashkey();
  unsigned int h1 = key % table_size;
  unsigned int h2 = 1 + key % ( table_size - 1 );
  unsigned int h = h1;
#else
...
// the state-insertion is done in two steps: search and insertion
  ...
  // JESSE: this copy_state() call causes a serious memory leak in preach...
  //   and apparently is not necessary for preach, thankfully
