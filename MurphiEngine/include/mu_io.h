...
public:
  Error_handler()
  : num_errors(0), phase(1) {buffer[0] = '\0';};
  ~Error_handler() {};
...
private:
  ...
  unsigned int inBuf;    // number of state in buffer (0: empty)
  unsigned int last;     // number of last state written
  void writeLong(unsigned int l, int bytes);
  unsigned int readLong(int bytes);

public:
  TraceFileManager(char*);
  ~TraceFileManager();
  void setBytes(int bits);
  unsigned int numLast();
  void write(unsigned int c1, unsigned int c2, unsigned int previous);
  const Buffer* read(unsigned int number);
};
#endif
