...
load_balancer(Names,Ratio,State) ->
	...
	idle ->
	    ...
	    io:format("LB: got im_feeling_idle from ~w~n",[IdlePid]),
	...
	{collecting,IdlePid,0,MaxLen,MaxPid} ->
	    if (MaxLen > 50) -> MaxPid ! {send_some_of_your_wq_to,IdlePid,Ratio};
	       true -> IdlePid ! {extraStateList,[]}
	    end,
...
start(P) ->
	...
	_ -> 
	    ...
	    tryToSendStates(0, CanonicalizedStartStates, Names, initBov(Names)),
...
initThreads(Names, NumThreads) ->
    ...
    if Local ->
            Args = [model_name(), UseSym,infinity, null, HashSize,CheckDeadlocks,PR],
            ID = spawn(preach,startWorker,Args);
       true ->
            Args = [model_name(), UseSym,BoBound,UnboBound, HashSize,CheckDeadlocks,PR ],
...
detectTermination(NumDone, GlobalSent, GlobalRecd, NumStates, Names,ProbDict) ->
    io:format("ROOT: NumDone = ~w, GlobalSent = ~w, GlobalRecd = ~w~n",[NumDone,GlobalSent,GlobalRecd]),
...
findPrev(Index, TraceFile) ->
    diskq:foldl(
      fun ({Sta, PrevSta}, X) -> if (Index == Sta) -> PrevSta; true -> X end end,
      not_found,
      TraceFile).
...
traceMode(Names, TraceFile, TraceH, UseSym) ->
    receive 
        {find_prev, State, Trace} ->
            PrevState = findPrev(State, TraceFile),
            T2 = [State | Trace],
            if PrevState == 0 ->
                    io:format("------------------~n",[]),
