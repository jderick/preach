...
/* StateManager */
/************************************************************/
StateManager::StateManager(bool createqueue, unsigned int NumStates)
: NumStates(NumStates),
  ...
  pno(1.0)
{
  if (createqueue) 
    { 
      queue = new state_queue((unsigned int) (gPercentActiveStates * NumStates) );
    }
  else 
    { 
      queue = new state_stack((unsigned int) (gPercentActiveStates * NumStates) );
    }
...
unsigned int StateManager::NumElts()
{ 
  return the_states->NumElts();
} 

unsigned int StateManager::NumEltsReduced()
{
  return the_states->NumEltsReduced();
}

unsigned int StateManager::QueueNumElts()
{ 
...
#ifdef ERLANG
int StartStateManager::AllStartStatesList(char* l)
{
  state *nextstate = NULL;
  int i = 0;
  ...
  for(what_startstate=0; what_startstate<numstartstates; what_startstate++) {
    memset(workingstate->bits, 0, BLOCKS_IN_WORLD);

    nextstate = StartState();   // nextstate points to internal data at theworld.getstate()
    ei_encode_list_header(l, &i, 1);
#ifdef RETURN_RULE_NUM_PAIRED_WITH_STATE
    ei_encode_tuple_header(l, &i, 2);
    ei_encode_long(l, &i, what_startstate);
#endif
...
int RuleManager::RuleName(char* l,unsigned rule )
{
  int i = 0;
  ei_encode_version(l, &i);
  ei_encode_string(l, &i, generator->Name(rule));
  return i;
}

int StartStateManager::StartStateName(char* l,unsigned ss_num )
{
  int i = 0;
  ei_encode_version(l, &i);
  ei_encode_string(l, &i, generator->Name(ss_num));
  return i;
}

int RuleManager::NextStatePreach(char* s, char* l,unsigned rule )
{
  state *nextstate = NULL;
  ...
  if (fire->in(what_rule)) {
    ...
    printf("MurphiEngine error: firing rule when not enabled\n");
    exit(1);
  }
  return i;
}

// copy all next states of s into buffer l which is size bytes, return # of states
int RuleManager::AllNextStatesList(char* s, char* l)
{
#ifndef POINTER_TRICK
  memcpy(workingstate->bits, s, BLOCKS_IN_WORLD);
#else
  workingstate->bits = (unsigned char*)s;
#endif
 // memset(workingstate->bits, 0, BLOCKS_IN_WORLD);
    ...
    if (fire->in(what_rule) && generator->Priority(what_rule)<=minp) {
      ...
      if (Error.NoError())  {
         if ( StateCmp(&origstate,nextstate)!=0) {
	       ...
	       ei_encode_binary(l, &i, nextstate->bits, BLOCKS_IN_WORLD); 
#ifdef POINTER_TRICK
	       workingstate->bits = origstate.bits;
           workingstate->previous = origstate.previous;
#else
...
RuleManager::NextState()
{
  ...
  // fire rule
  //if (what_rule==0) {printf("what_rule==0 \n");fflush(0);}
  generator->Code(what_rule);
  rules_fired++;
...
unsigned int RuleManager::NumRulesFired()
{
  return rules_fired;
}
...
#ifdef ERLANG
int 
PropertyManager::CheckInvariantsErl(char* s, char* out)
{
#ifndef POINTER_TRICK
  memcpy(workingstate->bits, s, BLOCKS_IN_WORLD);
#else
  ...
  for ( what_invariant = 0; what_invariant < numinvariants; what_invariant++ )
    {
      if ( !( *invariants[ what_invariant ].condition )() )
        /* Uh oh, invariant blown. */
        {
	  ei_encode_tuple_header(out, &i, 2);
	  ei_encode_atom(out, &i, "fail");
	  ei_encode_string(out, &i,invariants[what_invariant].name);
	  return i;
        }
    }
  ei_encode_atom(out, &i, "pass");
  return i;
}
int 
PropertyManager::is_p_state(char* s, char* out)
{
  memcpy(workingstate->bits, s, BLOCKS_IN_WORLD);
  ...
  category = INVARIANT;
  if (!livenesses) {
    printf("PropertyManager::is_p_state invoked with livenesses NULL, dying\n");
    exit(1);
  }
  if ( ! livenesses->condition_left() ) {
   // ei_encode_tuple_header(out, &i, 2);
    ei_encode_atom(out, &i, "false");
    //ei_encode_string(out, &i, "");
  } else {
    ei_encode_atom(out, &i, "true");
  }
  return i;
}
int 
PropertyManager::is_q_state(char* s, char* out)
{
  memcpy(workingstate->bits, s, BLOCKS_IN_WORLD);
  ...
  //category = INVARIANT;
  if (!livenesses) {
    printf("PropertyManager::is_q_state invoked with livenesses NULL, dying\n");
    exit(1);
  }
  if ( ! livenesses->condition_right() ) {
    //ei_encode_tuple_header(out, &i, 2);
    ei_encode_atom(out, &i, "false");
    //ei_encode_string(out, &i, "");
  } else { 
...
int 
PropertyManager::model_name(char* s, char* out)
{
  int i = 0;
  ei_encode_version(out, &i);
  ei_encode_string(out, &i, PROTOCOL_NAME);
  return i;
}
int 
PropertyManager::has_cangetto(char* s, char* out)
{
  memcpy(workingstate->bits, s, BLOCKS_IN_WORLD);
  ...
  // JESSE: not sure if this is needed (ditto for above two functions)
  //category = INVARIANT;
  if (numlivenesses > 0) {
    ei_encode_atom(out, &i, "true");
  } else {
...
int 
PropertyManager::StateToString(char* s, char* out)
{
  memcpy(workingstate->bits, s, BLOCKS_IN_WORLD);

  int i = 0;
  ei_encode_version(out, &i);
  string str = (string)theworld;
  ei_encode_string(out, &i, ((string)theworld).c_str());
    
  return i;
}

int 
PropertyManager::PrintDiff(char* s, char* out)
{
  state prev;
  prev = *workingstate;
  memcpy(prev.bits, s, BLOCKS_IN_WORLD); 
  memcpy(workingstate->bits, s+BLOCKS_IN_WORLD, BLOCKS_IN_WORLD);
 

  theworld.print_diff(&prev);
  cout << endl;

  int i = 0;
  ei_encode_version(out, &i);
  ei_encode_string(out, &i, "ok");
    
  return i;
}

int 
RuleManager::whatRuleFired(char* s, char* out)
{
  state nextState;
  state *tempState = 0;
  memcpy(workingstate->bits, s, BLOCKS_IN_WORLD);
  memcpy(nextState.bits, s+BLOCKS_IN_WORLD, BLOCKS_IN_WORLD);
  setofrules * fire = EnabledTransition();
  int i = 0; 
  state origstate(workingstate);
  ...
  for ( what_rule=0; what_rule<numrules; what_rule++) {
    if (fire->in(what_rule)) {
      tempState = NextState();
      if ( StateCmp(&nextState,tempState)==0) {
        ei_encode_long(out, &i, what_rule);
        return i;
      }
	  *workingstate = origstate;
    } 
  }
  // Use -1 to indicate that the two states are not a valid transition
  ei_encode_long(out, &i, -1);
  return i;
}
